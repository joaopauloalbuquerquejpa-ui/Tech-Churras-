...
exports.whatsappWebhookRoutes = whatsappWebhookRoutes;
exports.sendFollowUps = sendFollowUps;
...
async function zapiSend(phone, message) {
    ...
    try {
        ...
        if (!res.ok)
            console.error('[WhatsApp] Z-API error:', res.status);
    }
    catch (err) {
        console.error('[WhatsApp] send failed:', err?.message);
    }
}
async function notifyAdmin(leadInfo, phone) {
    const adminPhone = process.env.ADMIN_WHATSAPP_PHONE;
    if (!adminPhone)
        return;
    await zapiSend(adminPhone, `🔥 *LEAD QUALIFICADO — AÇOUGUE SP*\n\n${leadInfo}\nTel: wa.me/55${phone.replace(/\D/g, '')}\n\n_Acesse o painel: techchurras.com.br/admin_`);
}
async function saveLead(phone, info) {
    ...
    try {
        await prisma_1.prisma.lead.upsert({
            where: { phone },
...
async function markLeadContacted(phone) {
    try {
        await prisma_1.prisma.lead.upsert({
            where: { phone },
...
async function whatsappWebhookRoutes(app) {
    ...
    app.post('/webhooks/whatsapp', {
        ...
        prisma_1.prisma.lead.updateMany({
            where: { phone },
    ...
    app.get('/webhooks/leads', async (request, reply) => {
        ...
        if (token !== process.env.WEBHOOK_SECRET)
            return reply.status(401).send({ error: 'Unauthorized' });
        const leads = await prisma_1.prisma.lead.findMany({ orderBy: { createdAt: 'desc' }, take: 100 });
        return reply.send(leads);
    });
}
// ── Follow-up automático 48h — chamado pelo cron
async function sendFollowUps() {
    const due = await prisma_1.prisma.lead.findMany({
        where: {
    ...
    if (due.length > 0)
        console.log(`[Follow-up] ${due.length} mensagens enviadas`);
}
