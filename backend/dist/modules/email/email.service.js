...
exports.emailOrderConfirmed = emailOrderConfirmed;
exports.emailNewOrderGrillmaster = emailNewOrderGrillmaster;
exports.emailOrderCompleted = emailOrderCompleted;
exports.emailPartnerApproved = emailPartnerApproved;
exports.emailWelcomeCustomer = emailWelcomeCustomer;
...
async function sendEmail(to, subject, html, label) {
    ...
    try {
        ...
        if (!res.ok)
            console.log(`[Email] ${label} erro:`, res.status);
        else
            console.log(`[Email] ${label} enviado para`, to);
    }
    catch (err) {
...
async function emailOrderConfirmed(to, customerName, orderId, grillmasterName, eventDate, eventAddress) {
...
async function emailNewOrderGrillmaster(to, grillmasterName, orderId, customerName, eventDate, guestCount) {
...
async function emailOrderCompleted(to, customerName, orderId, grillmasterName) {
    const firstName = customerName.split(' ')[0];
...
async function emailPartnerApproved(to, partnerName, partnerType, dashUrl) {
...
async function emailWelcomeCustomer(to, customerName) {
    const firstName = customerName.split(' ')[0];
    const html = baseTemplate(`
