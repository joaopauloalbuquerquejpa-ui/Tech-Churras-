...
exports.generateContractSchema = void 0;
...
exports.generateContractSchema = zod_1.z.object({
    ...
    partnerDocument: zod_1.z.string().min(11, 'Informe CPF ou CNPJ'),
    partnerAddress: zod_1.z.string().optional(),
});
async function generateContract(userId, data) {
    ...
    const existing = await prisma_1.prisma.contract.findFirst({
        where: { partnerId, status: 'ACCEPTED' },
    });
    if (existing)
        throw new Error('Já existe um contrato aceito para este parceiro');
...
async function acceptContract(contractId, userId, ip, userAgent) {
    const user = await prisma_1.prisma.user.findUnique({
        where: { id: userId },
    ...
    if (contract.partnerId !== ownedId)
        throw new Error('Sem permissão para aceitar este contrato');
    if (contract.status === 'ACCEPTED')
        throw new Error('Contrato já foi aceito');
...
async function getMyContracts(userId) {
    const user = await prisma_1.prisma.user.findUnique({
        where: { id: userId },
        include: { boutique: true, grillmaster: true },
    });
...
async function getContractById(contractId, userId) {
    ...
    const ownedId = user.boutique?.id ?? user.grillmaster?.id;
    if (!isAdmin && contract.partnerId !== ownedId)
